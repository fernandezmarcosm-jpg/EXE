...
func crearControles(hwnd uintptr) {
	...
	for i, mode := range modes {
		...
		if mode == mainConfig.Mode { selected = i }
	}
...
func crearBoton(hwnd uintptr, texto string, x, y, ancho, alto int, id uintptr) uintptr {
	h, _, _ := user32.NewProc("CreateWindowExW").Call(0, uintptr(unsafe.Pointer(u16("BUTTON"))), uintptr(unsafe.Pointer(u16(texto))), WS_CHILD|WS_VISIBLE|BS_PUSHBUTTON, uintptr(x), uintptr(y), uintptr(ancho), uintptr(alto), hwnd, id, hInstance, 0)
	return h
}
...
func redimensionarControles(hwnd uintptr) {
	...
	for _, b := range toolbar {
		...
		if bw < 85 { bw = 85 }
		if bw > 165 { bw = 165 }
		if ctl := getDlgItem(hwnd, b.id); ctl != 0 { user32.NewProc("MoveWindow").Call(ctl, uintptr(x), 6, uintptr(bw), 28, 1) }
		x += bw + 4
	}
	...
	for _, f := range labels {
		if lh := filterLabels[f.id]; lh != 0 { user32.NewProc("MoveWindow").Call(lh, uintptr(f.x), 42, 65, 20, 1) }
		if eh := filterHandles[f.id]; eh != 0 { user32.NewProc("MoveWindow").Call(eh, uintptr(f.x), 60, uintptr(f.width), 24, 1) }
	}
	if b := getDlgItem(hwnd, ID_FILTRAR); b != 0 { user32.NewProc("MoveWindow").Call(b, 690, 58, 85, 27, 1) }
	if b := getDlgItem(hwnd, ID_LIMPIAR); b != 0 { user32.NewProc("MoveWindow").Call(b, 780, 58, 85, 27, 1) }
	gridY, statusH, totalsH := 92, 28, 48
	gridH := h - gridY - statusH - totalsH - 8
	if gridH < 100 { gridH = 100 }
	if hwndGrid != 0 { user32.NewProc("MoveWindow").Call(hwndGrid, 8, uintptr(gridY), uintptr(w-16), uintptr(gridH), 1) }
	if hwndTotals != 0 { user32.NewProc("MoveWindow").Call(hwndTotals, 8, uintptr(h-statusH-totalsH), uintptr(w-16), uintptr(totalsH), 1) }
	if hwndStatus != 0 { user32.NewProc("MoveWindow").Call(hwndStatus, 8, uintptr(h-statusH), uintptr(w-16), uintptr(statusH), 1) }
}
...
func handleCommand(hwnd, wParam, lParam uintptr) uintptr {
	...
	if notify != BN_CLICKED && id != ID_MODO { return 0 }
	switch id {
...
func resolveUIValue(l Line, name string) string {
	if v := fieldValue(l, name); v != "" { return v }
	aliases := map[string][]string{"SKU":{"sku"}, "Descripción":{"descrip", "descripcion", "producto"}, "SUM (%) descuento":{"sum", "descuento", "% descuento"}, "NETO PK":{"neto pk", "neto_pk", "neto"}, "UNIDADES":{"unidades", "unidad"}, "PALL":{"pall", "pallets", "pallet"}, "PK":{"pk"}, "NETO SO":{"neto so"}, "TN SO":{"tn so", "tn"}, "CMG":{"cmg"}, "PPP SO":{"ppp so", "ppp"}, "ORIGEN":{"origen"}}
	for _, a := range aliases[name] { if v := findAnyValue(l, a); v != "" { return v } }
	return ""
}
func refreshGrid(lines []Line) {
	...
	for rowIndex, l := range lines {
		...
		for colIndex, c := range uiColumns { values[colIndex] = resolveUIValue(l, c.Name) }
		first := u16(values[0])
		item := lvitemw{Mask: LVIF_TEXT, Item: int32(rowIndex), SubItem: 0, Text: uintptr(unsafe.Pointer(first)), TextMax: int32(len(values[0])+1)}
		user32.NewProc("SendMessageW").Call(hwndGrid, LVM_INSERTITEMW, 0, uintptr(unsafe.Pointer(&item)))
		for colIndex := 1; colIndex < len(values); colIndex++ { setGridCell(rowIndex, colIndex, values[colIndex]) }
	}
}
