...
type openfilename struct { LStructSize uint32; HwndOwner uintptr; HInstance uintptr; Filter uintptr; CustomFilter uintptr; MaxCustFilter uint32; FilterIndex uint32; File uintptr; MaxFile uint32; FileTitle uintptr; MaxFileTitle uint32; InitialDir uintptr; Title uintptr; Flags uint32; FileOffset uint16; FileExtension uint16; DefExt uintptr; CustData uintptr; Hook uintptr; TemplateName uintptr; Reserved uintptr; Reserved2 uint32; FlagsEx uint32 }
...
func crearVentana() uintptr {
	...
	hwnd, _, _ := user32.NewProc("CreateWindowExW").Call(0, uintptr(unsafe.Pointer(className)), uintptr(unsafe.Pointer(syscall.StringToUTF16Ptr("Gestion SO V54 - SO RETENIDAS / CSV maestro"))), WS_OVERLAPPEDWINDOW|WS_VISIBLE, CW_USEDEFAULT, CW_USEDEFAULT, 1250, 780, 0, 0, hInstance, 0)
	if hwnd != 0 {
...
func windowTitle() string { mode := strings.TrimSpace(mainConfig.Mode); if strings.HasPrefix(mode, "MODO: ") { mode = strings.TrimSpace(strings.TrimPrefix(mode, "MODO: ")) }; if mode == "" { mode = "SO RETENIDAS" }; return "Gestion SO V54 - " + mode + " / CSV maestro" }
...
func crearBoton(hwnd uintptr, texto string, x,y,ancho,alto int,id uintptr) { user32.NewProc("CreateWindowExW").Call(0,uintptr(unsafe.Pointer(u16("BUTTON"))),uintptr(unsafe.Pointer(u16(texto))),WS_CHILD|WS_VISIBLE|WS_TABSTOP|BS_PUSHBUTTON,uintptr(x),uintptr(y),uintptr(ancho),uintptr(alto),hwnd,id,hInstance,0) }
...
func redimensionarControles(hwnd uintptr) {
	...
	x:=10; for _,b:=range toolbar {bw:=len([]rune(b.text))*8+22;if bw<85{bw=85};if bw>160{bw=160};if ctl:=getDlgItem(hwnd,b.id);ctl!=0{user32.NewProc("MoveWindow").Call(ctl,uintptr(x),8,uintptr(bw),28,1)};x+=bw+5};if hwndMode!=0{user32.NewProc("MoveWindow").Call(hwndMode,uintptr(x),8,190,28,1)}
	labels:=[]struct{id,x,width int}{{ID_FILTRO_SO,10,105},{ID_FILTRO_ESTADO,125,135},{ID_FILTRO_SKU,270,125},{ID_FILTRO_SUMA,405,135},{ID_FILTRO_SDSRP2,550,135}};for _,f:=range labels{if e:=filterHandles[f.id];e!=0{user32.NewProc("MoveWindow").Call(e,uintptr(f.x+55),45,uintptr(f.width),24,1)}};if b:=getDlgItem(hwnd,ID_FILTRAR);b!=0{user32.NewProc("MoveWindow").Call(b,695,44,85,26,1)};if b:=getDlgItem(hwnd,ID_LIMPIAR);b!=0{user32.NewProc("MoveWindow").Call(b,785,44,85,26,1)}
	gridY:=78;statusH:=26;totalsH:=42;gridH:=h-gridY-statusH-totalsH-12;if gridH<120{gridH=120};if hwndGrid!=0{user32.NewProc("MoveWindow").Call(hwndGrid,10,uintptr(gridY),uintptr(w-20),uintptr(gridH),1)};if hwndTotals!=0{user32.NewProc("MoveWindow").Call(hwndTotals,10,uintptr(gridY+gridH+3),uintptr(w-20),uintptr(totalsH),1)};if hwndStatus!=0{user32.NewProc("MoveWindow").Call(hwndStatus,10,uintptr(h-statusH),uintptr(w-20),uintptr(statusH),1)}
}
...
func handleCommand(hwnd,wParam,lParam uintptr)uintptr{_ = lParam;id:=int(wParam&0xffff);notify:=uint16((wParam>>16)&0xffff);switch id{case ID_ABRIR_XLSX:if notify==BN_CLICKED{openXLSXDialog(hwnd)};case ID_RECARGAR:if notify==BN_CLICKED{updateMainView(hwnd);logf("UI stub: RECARGAR")};case ID_COLUMNAS:if notify==BN_CLICKED{logf("UI stub: COLUMNAS...")};case ID_FILTRAR:if notify==BN_CLICKED{applyHeaderFilters(hwnd)};case ID_LIMPIAR:if notify==BN_CLICKED{clearHeaderFilters(hwnd)};case ID_EXPORTAR_CSV:if notify==BN_CLICKED{logf("UI stub: EXPORTAR CSV")};case ID_TOMAR_EXCEL:if notify==BN_CLICKED{logf("UI stub: TOMAR EXCEL ABIERTO (COM pendiente)")};case ID_FILTROS_CAB:if notify==BN_CLICKED{logf("UI stub: FILTROS CABECERA...")};case ID_SIMULADOR:if notify==BN_CLICKED{openSimulator(hwnd);logf("UI stub: SIMULADOR")};case ID_RESALTAR:if notify==BN_CLICKED{logf("UI stub: RESALTAR...")};case ID_COLOR:if notify==BN_CLICKED{logf("UI stub: +/- COLOR...")};case ID_DATOS_CSV:if notify==BN_CLICKED{logf("UI stub: DATOS CSV...")};case ID_MODO:if notify==CBN_SELCHANGE{saveSelectedMode(hwnd)}};return 0}
func saveSelectedMode(hwnd uintptr){if hwndMode==0{return};idx,_,_:=user32.NewProc("SendMessageW").Call(hwndMode,CB_GETCURSEL,0,0);modes:=[]string{"MODO: FACTURAS PENDIENTES","MODO: SO RETENIDAS","MODO: FACTURAS"};if int(idx)<0||int(idx)>=len(modes){return};mainConfig.Mode=modes[int(idx)];if err:=SaveConfig(mainConfig);err!=nil{logf("SaveConfig Mode error: %v",err)};setWindowText(hwnd,windowTitle());updateMainView(hwnd)}
func applyHeaderFilters(hwnd uintptr){filters:=map[string]string{};currentFilterCount=0;for _,id:=range []int{ID_FILTRO_SO,ID_FILTRO_ESTADO,ID_FILTRO_SKU,ID_FILTRO_SUMA,ID_FILTRO_SDSRP2}{v:=strings.TrimSpace(windowText(filterHandles[id]));if v!=""{filters[filterName(id)]=v;currentFilterCount++}};currentView=BuildFilteredSortedViewByHeaders(mainLines,filters);refreshGrid(currentView);updateStatus(hwnd,currentView)}
...
func refreshGrid(lines []Line){if hwndGrid==0{return};user32.NewProc("SendMessageW").Call(hwndGrid,LVM_DELETEALLITEMS,0,0);for _,l:=range lines{row:=insertGridRow(resolveUIValue(l,uiColumns[0].Name));for i:=1;i<len(uiColumns);i++{setGridCell(row,i,resolveUIValue(l,uiColumns[i].Name))}};for _,s:=range CalculateSOSubtotals(lines){row:=insertGridRow(FormatSOSubtotal(s));for i:=1;i<len(uiColumns);i++{setGridCell(row,i,"")}}}
...
func openXLSXDialog(owner uintptr){files:=pickMultipleXLSX(owner);if len(files)==0{return};rows,err:=mergeXLSX(files);if err!=nil{logf("mergeXLSX error: %v",err);return};mainLines=BuildLines(rows,strings.Join(files,";"));currentFilterCount=0;currentView=BuildFilteredSortedViewByHeaders(mainLines,nil);refreshGrid(currentView);updateStatus(owner,currentView);for _,f:=range files{feedEngineFile(owner,f)}}
func pickMultipleXLSX(owner uintptr)[]string{buf:=make([]uint16,32768);filter:=u16z("Archivos XLSX (*.xlsx)\x00*.xlsx\x00Todos los archivos (*.*)\x00*.*\x00\x00");title:=u16("ABRIR XLSX");of:=openfilename{LStructSize:uint32(unsafe.Sizeof(openfilename{})),HwndOwner:owner,Filter:uintptr(unsafe.Pointer(&filter[0])),File:uintptr(unsafe.Pointer(&buf[0])),MaxFile:uint32(len(buf)),Title:uintptr(unsafe.Pointer(title)),Flags:OFN_EXPLORER|OFN_ALLOWMULTISELECT|OFN_FILEMUSTEXIST|OFN_HIDEREADONLY|OFN_ENABLEHOOK,Hook:syscall.NewCallback(multiSelectHook)};r,_,_:=user32.NewProc("GetOpenFileNameW").Call(uintptr(unsafe.Pointer(&of)));if r==0{return nil};return parseMultiSelectBuffer(buf)}
