...
func parseRows(rows []struct{Cells []struct{Ref string `xml:"r,attr"`;Type string `xml:"t,attr"`;Value string `xml:"v"`;Inline struct{T string `xml:"t"`} `xml:"is"`} `xml:"c"`},ss []string)[][]string{out:=[][]string{};for _,r:=range rows{row:=[]string{};for _,c:=range r.Cells{val:=c.Value;if c.Type=="s"{if i,err:=strconv.Atoi(val);err==nil&&i>=0&&i<len(ss){val=ss[i]}}else if c.Inline.T!=""{val=c.Inline.T};row=append(row,val)};out=append(out,row)};return normalizeRows(out)}
...
func mergeXLSX(paths []string)([][]string,error){var all [][]string;for _,p:=range paths{d,e:=ReadXLSX(p);if e!=nil{return nil,e};for _,rows:=range d.Sheets{all=append(all,rows...);break}};return all,nil}
...
func ensureMasterHeaders(path string,headers []string)error{f,e:=os.OpenFile(path,os.O_CREATE|os.O_WRONLY|os.O_APPEND,0644);if e!=nil{return e};defer f.Close();st,e:=f.Stat();if e!=nil{return e};if st.Size()==0{w:=csv.NewWriter(f);_ = w.Write(headers);w.Flush();return w.Error()};return nil}
func LoadMaster(path ...string)(*MasterData,error){p:=locateMaster();if len(path)>0&&path[0]!=""{p=path[0]};f,e:=os.Open(p);if e!=nil{return nil,e};defer f.Close();r:=csv.NewReader(f);rows,e:=r.ReadAll();if e!=nil{return nil,e};md:=&MasterData{Path:p};if len(rows)>0{md.Headers=rows[0];for i:=1;i<len(rows);i++{row:=rows[i];mr:=MasterRow{};for j,h:=range md.Headers{if j<len(row){mr[h]=row[j]}else{mr[h]=""}};md.Rows=append(md.Rows,mr)}};return md,nil}
...
func BuildLines(rows [][]string,source string)[]Line{if len(rows)==0{return nil};hi:=headerRowIndex(rows);headers:=uniqueHeaders(rows[hi]);out:=make([]Line,0,len(rows)-hi-1);for i:=hi+1;i<len(rows);i++){values:=map[string]string{};for j,v:=range rows[i]{if j<len(headers){values[headers[j]]=v}};out=append(out,Line{Values:values,Source:source,RowNumber:i})};return out}
...
func availableColumns(lines []Line)[]ColumnDef{if len(lines)==0{return nil};keys:=make([]string,0,len(lines[0].Values));for k:=range lines[0].Values{keys=append(keys,k)};sort.Strings(keys);o:=make([]ColumnDef,0,len(keys));for _,k:=range keys{o=append(o,ColumnDef{Name:k,Width:100})};return o}
...
func parseNumber(s string)(float64,bool){s=strings.TrimSpace(s);if s==""{return 0,false};s=strings.ReplaceAll(s,"$","");s=strings.ReplaceAll(s," ","");if strings.Contains(s,",")&&strings.Contains(s,"."){s=strings.ReplaceAll(s,",","")};if x,err:=strconv.ParseFloat(s,64);err==nil{return x,true};return 0,false}
...
func CalculateSOSubtotals(lines []Line)[]GroupSubtotal{groups:=GroupLines(lines);keys:=make([]string,0,len(groups));for k:=range groups{keys=append(keys,k)};sort.Strings(keys);o:=make([]GroupSubtotal,0,len(keys));for _,k:=range keys{g:=groups[k];var s GroupSubtotal;s.Key=k;s.Lines=len(g);for _,l:=range g{s.Bultos+=numericByNames(l,"bultos","bultos" );s.Unidades+=numericByNames(l,"unidades","unidades");s.Neto+=numericByNames(l,"neto","importe")}o=append(o,s)};return o}
func FormatSOSubtotal(s GroupSubtotal)string{return fmt.Sprintf("SUBTOTAL SO %s | RET %d | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s | %s",s.Key,s.Lines,"","","","",formatNumber(s.Unidades),"","","","","","")}
