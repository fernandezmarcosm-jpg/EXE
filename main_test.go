...
func TestDuplicatedPhysicalColumns(t *testing.T) {
	...
	if r.Values["DESCRIPCION_2"].Raw == r.Values["DESCRIPCION"].Raw {
		t.Fatalf("duplicated description values are not independent")
	}
	if got := r.Values["DESCRIPCION_2"].Raw; got != "segunda" {
		t.Fatalf("second description = %q, want %q", got, "segunda")
	}
}
