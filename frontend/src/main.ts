...
function renderBody(){
  ...
  if(activeSubtotal){const total=(state.data.subtotals??[]).find(sr=>sr.total);if(total)bodyParts.push('<tr class="subtotal subtotal-total">'+cols.map((c,i)=>cellMarkup(c,total.values[c.id]??(i===0?total.group_value:'')).replace('class=""','')).join('')+'</tr>')}
...
function renderSubtotalControls(){if(!state.data)return;const current=state.visual.settings?.subtotal_column??'';subtotalGroup.innerHTML='<option value="">Sin subtotales</option>'+state.data.columns.map(c=>'<option value="'+escAttr(c.id)+'" '+(c.id===current?'selected':'')+'>'+esc(c.title)+'</option>').join('');const agg=state.visual.settings?.subtotal_agg??{};subtotalFields.innerHTML=state.data.columns.filter(c=>c.id!==current).map(c=>{const numeric=isNumericColumn(c);return '<label class="subtotal-field">'+esc(c.title)+' <select data-subtotal-id="'+escAttr(c.id)+'"><option value="">Nada</option>'+(numeric?'<option value="suma">Suma</option><option value="promedio">Promedio</option>':'')+'<option value="conteo_unico">Contador (únicos)</option></select></label>'}).join('');subtotalFields.querySelectorAll<HTMLSelectElement>('select[data-subtotal-id]').forEach(s=>{s.value=agg[s.dataset.subtotalId!]??'';s.addEventListener('change',saveSubtotals)})}
