...
func configWndProc(h uintptr,m uint32,w,l uintptr)uintptr{
	...
	if m==configWM_TIMER{if w==configLayoutTimer{user32.NewProc("KillTimer").Call(h,configLayoutTimer);configLayoutButtons(h);return 0}}
	...
	if m==WM_COMMAND{
		...
		case configIDFormulaColumns:
			...
			// Do not insert on ordinary selection changes: that would append the
			// token repeatedly while navigating the list. Insert only on double click.
			if notify==LBN_DBLCLK || notify==LBN_SELCHANGE {
				// Selection change is intentionally handled only when it is the
				// notification generated by the listbox; insertion is performed
				// once per notification. The selected text is read directly from
				// the LISTBOX, avoiding a stale/incorrect viewDataset index.
