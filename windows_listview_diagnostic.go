...
func diagCall(proc *syscall.LazyProc, args ...uintptr) (uintptr, uintptr, syscall.Errno) {
...
func diagBool(name string, hwnd uintptr, p *syscall.LazyProc) {
    v, _, _ := diagCall(p, hwnd)
    appLog("LISTVIEW %s=%d", name, v)
}
...
func diagLogChildren(parent uintptr, list uintptr) {
    appLog("LISTVIEW CHILDREN_BEGIN parent=0x%X target=0x%X", parent, list)
    ...
    for i:=0; child!=0 && i<256; i++ {
        ...
        appLog("LISTVIEW CHILD[%d] HWND=0x%X CLASS=%q TEXT=%q ID=%d VISIBLE=%d STYLE=0x%X EXSTYLE=0x%X RECT=%s ZPREV=0x%X ZNEXT=0x%X",
            i,child,diagClass(child),diagText(child),windowID(child),boolNum(isWindowVisible(child)),style,ex,diagRectString(r,true),prevWindow(child),nextWindow(child))
...
func diagReadItem(row int) string {
    return diagReadCell(row,0)
}
...
func columnViewDeepDiagnostic(stage string) {
    ...
    for i:=0;i<int(count)&&i<64;i++ { appLog("LISTVIEW ROW[%d] COL0=%q",i,diagReadItem(i)); for c:=0;c<int(hcount)&&c<32;c++ { appLog("LISTVIEW CELL row=%d col=%d text=%q",i,c,diagReadCell(i,c)) } }
