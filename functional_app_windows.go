...
func appPickXLSX(owner uintptr) []string {
    ...
    buffer:=make([]uint16,65536); title:=appU16("Seleccionar archivo Excel"); defExt:=appU16("xlsx")
    ...
    for pos<len(buffer) {end:=pos;for end<len(buffer)&&buffer[end]!=0{end++};if end==pos{break};s:=strings.TrimSpace(syscall.UTF16ToString(buffer[pos:end]));if s!=""{parts=append(parts,s)};pos=end+1}
    if len(parts)==0{return nil}
    if len(parts)>1 {dir:=parts[0];out:=make([]string,0,len(parts)-1);for _,name:=range parts[1:]{if filepath.IsAbs(name){out=append(out,name)}else{out=append(out,filepath.Join(dir,name))}};appLog("EVENTO: selector devolvio %d archivos",len(out));return out}
...
func loadPreviewXLSX(path string)([]string,[]appRow,string,error){
    doc,err:=ReadXLSX(path);if err!=nil{return nil,nil,"",err};if len(doc.Sheets)==0{return nil,nil,"",fmt.Errorf("el Excel no contiene hojas legibles")}
    names:=make([]string,0,len(doc.Sheets));for n:=range doc.Sheets{names=append(names,n)};sort.Strings(names);rows:=doc.Sheets[names[0]]
    if len(rows)==0{return nil,nil,"",fmt.Errorf("la hoja %q no contiene filas de datos",names[0])}
    n:=maxPreviewColumns(rows);headers:=makeHeaders(rows[0],n);const maxRows=5000;if len(rows)-1>maxRows{rows=rows[:maxRows+1]}
    out:=make([]appRow,0,len(rows)-1);for i:=1;i<len(rows);i++{vals:=make([]string,n);for j:=0;j<n;j++{if j<len(rows[i]){vals[j]=rows[i][j]}};out=append(out,appRow{Values:vals})};return headers,out,path,nil
}
