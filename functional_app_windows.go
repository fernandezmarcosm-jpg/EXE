...
// Win64 OPENFILENAMEW layout. Padding after DWORD fields is intentional.
...
func appMake(parent uintptr, cls, text string, style uint32, x, y, w, h int, id uintptr) uintptr {
    c := appU16(cls)
    t := appU16(text)
    r, _, _ := user32.NewProc("CreateWindowExW").Call(
        0, uintptr(unsafe.Pointer(c)), uintptr(unsafe.Pointer(t)), uintptr(style),
...
func crearVentana() uintptr {
    ...
    wc := appWndClass{
        ...
        HCursor: func() uintptr {
            r, _, _ := user32.NewProc("LoadCursorW").Call(0, 32512)
            return r
        }(),
        LpszClassName: cls,
    }
    ...
    h, _, _ := user32.NewProc("CreateWindowExW").Call(
        0, uintptr(unsafe.Pointer(cls)), uintptr(unsafe.Pointer(title)),
        WS_OVERLAPPEDWINDOW|WS_VISIBLE, 0x80000000, 0x80000000, 1400, 820,
        0, 0, appHInstance, 0,
    )
    return h
}
...
func appPickXLSX(owner uintptr) []string {
    ...
    ofn := appOpenFile{
        ...
        Flags: ofnExplorer|ofnFileMustExist|ofnPathMustExist|ofnHideReadOnly|ofnAllowMultiSelect,
    ...
    for pos < len(buffer) {
        ...
        for end < len(buffer) && buffer[end] != 0 { end++ }
        if end == pos { break }
        s := strings.TrimSpace(syscall.UTF16ToString(buffer[pos:end]))
        if s != "" { parts = append(parts, s) }
        pos = end + 1
    }
    if len(parts) == 0 { return nil }

    // With OFN_ALLOWMULTISELECT: first item is the directory when there is
    // more than one item; subsequent items are file names.
    if len(parts) > 1 {
        dir := parts[0]
        out := make([]string, 0, len(parts)-1)
        for _, name := range parts[1:] {
            if filepath.IsAbs(name) { out = append(out, name) } else { out = append(out, filepath.Join(dir, name)) }
        }
        appLog("EVENTO: selector devolvio %d archivos", len(out))
        return out
    }
    appLog("EVENTO: selector devolvio 1 archivo")
...
func appOpenXLSX(owner uintptr) {
    ...
    go func() {
        ...
        appLog("EVENTO: inicio lectura XLSX; archivos=%d", len(selected))
...
func loadPreviewXLSX(path string) ([]string, []appRow, string, error) {
    ...
    for n := range doc.Sheets { names = append(names, n) }
    sort.Strings(names)
    ...
    // Keep the preview bounded. The original XLSX is never modified.
    const maxRows = 5000
    ...
    for i := 1; i < len(rows); i++ {
        vals := make([]string, n)
        for j := 0; j < n; j++ {
            if j < len(rows[i]) { vals[j] = rows[i][j] }
        }
...
func maxPreviewColumns(rows [][]string) int {
    n := 0
    for _, r := range rows { if len(r) > n { n = len(r) } }
    if n > 80 { n = 80 }
    return n
}

func makeHeaders(row []string, n int) []string {
    out := make([]string, n)
    seen := map[string]int{}
    for i := 0; i < n; i++ {
        name := "Columna " + fmt.Sprint(i+1)
        if i < len(row) && strings.TrimSpace(row[i]) != "" { name = strings.TrimSpace(row[i]) }
        base := name
        seen[base]++
        if seen[base] > 1 { name = fmt.Sprintf("%s (%d)", base, seen[base]) }
        out[i] = name
    }
    return out
}
...
func renderPreview(headers []string, rows []appRow) string {
    ...
    // Hard UI cap prevents a malformed/huge workbook from monopolizing the
    // Windows EDIT control. Data remains in the original file.
    const maxChars = 2 * 1024 * 1024
    for i, h := range headers {
        if i > 0 { b.WriteByte('\t') }
        b.WriteString(cleanCell(h))
    }
    b.WriteString("\r\n")
    for _, row := range rows {
        for i := range headers {
            if i > 0 { b.WriteByte('\t') }
            if i < len(row.Values) { b.WriteString(cleanCell(row.Values[i])) }
        }
        b.WriteString("\r\n")
...
func appRefreshView() {
    appStateMu.Lock()
    status := appStatusText
    view := appViewText
    appStateMu.Unlock()
    appSetText(appStatus, status)
    appSetText(appView, view)
}
